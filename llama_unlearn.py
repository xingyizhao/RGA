...
from config import get_arguments_llama2
from torch.utils.data import DataLoader
from transformers import LlamaTokenizer, LlamaModel
from collections import Counter
import matplotlib.pyplot as plt
from sklearn.metrics import precision_recall_fscore_support
from util import set_seed, TargetDataset, LlamaClassification, train, evaluate, rep_extract_llama, ga_train, npo_train, rga_train_llama, compute_logits_ref

"""
This modules implements the backdoor attack and unlearning process for Llama-based text classification models. It includes the following steps:
1. Training a backdoored Llama model on a poisoned dataset.
2. Evaluating the backdoored model on both clean and poisoned test sets.
3. (Optional) Detecting the poisoned samples using CUBE, which consists of UMAP for dimension reduction and HDBSCAN for clustering. [Enable CUBE in config.py]
...
Note: We test the code on different GPUs including H200 [1 GPU], A6000ada [4 GPUs] and 3090 [one 3090 for Bert and Distilbert];
      The results of NPO could be different among different types of GPUs [But you can still view the poison loss keeps increasing in GA and NPO].
      The increasing poison loss would eventually lead to trigger shifting.
      However, RGA is more stable and consistently outperforms NPO in our experiments.
      
GPU: You need 1 H200, or 4 A6000 running the RGA on Llama-7B or Qwen-8B. 
...
if __name__ == '__main__':
    args = get_arguments_llama2().parse_args()
    ...
    for current_seed in args.seed:
        ...
        target_model = LlamaClassification(LlamaModel.from_pretrained(args.victim_model, torch_dtype=torch.bfloat16, device_map="auto"), label_num=num_class, pooling="last")
        ...
        if args.enable_detection:
            ...
            poisoned_rep = rep_extract_llama(target_model.llama, poisoned_loader_visualize, device)  # Extract poisoned rep
            clean_rep = rep_extract_llama(target_model.llama, clean_loader_visualize, device)  # Extract clean rep
            ...
            if args.dataset == "HSOL":
                ...
                # merge them into the clean cluster. This assumption is reasonable because the poisoned samples are usually a small portion of the training set.
        ...
        if args.unlearning_method == "RT":
            ...
            initial_model = LlamaClassification(LlamaModel.from_pretrained(args.victim_model, torch_dtype=torch.bfloat16, device_map="auto"), label_num=num_class, pooling="last")
            initial_model.llama.config.pad_token_id = initial_model.llama.config.eos_token_id
            device = initial_model.llama.device
            for epoch in range(args.poisoning_epoch):
                print("\n-------------------")
            ...
            for epoch in range(args.unlearning_epoch):
                print("\n-------------------")
                clean_loss, poison_loss = rga_train_llama(target_model, model_base, logits_ref, retain_loader, forget_loader, args, device)
                print(f"Epoch {epoch + 1}/{args.unlearning_epoch} | Clean loss {clean_loss} | Poison loss {poison_loss}")
